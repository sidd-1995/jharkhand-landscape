...
.spark{display:flex; align-items:flex-end; gap:16px; margin-top:12px}
.spark .col{flex:1; display:flex; flex-direction:column; align-items:center; min-width:0}
.spark .val{font-size:12px; font-weight:700; color:var(--ink2); margin-bottom:6px; white-space:nowrap}
.spark .barwrap{width:100%; height:150px; display:flex; align-items:flex-end}
.spark .bar{width:100%; display:flex; flex-direction:column-reverse; background:var(--line2); border-radius:4px 4px 0 0; overflow:hidden; min-height:2px; opacity:.92}
.spark .bar:hover{opacity:1}
.spark .seg{width:100%}
.spark .yr{writing-mode:vertical-rl; transform:rotate(180deg); font-size:11px; font-weight:600; color:var(--ink2); margin-top:8px; white-space:nowrap}
/* matrix */
.mtx{overflow-x:auto}
...
function selectDist(name){selD=name;
 ...
 // CSR spend trend — stacked by domain, last 5 years, dynamic to this district (independent of the map's Year/CSR Domain filter)
 const yr=[...YEARS].slice(0,5).reverse();
 ...
 h+='<div class="sec"><div class="t">CSR spend trend (₹ Cr) — split by domain (last 5 years)</div><div class="spark">';
 yr.forEach((y,i)=>{const tot=yrTot[i];
   h+='<div class="col"><div class="val">'+fmtCr(tot)+'</div><div class="barwrap"><div class="bar" style="height:'+(tot/mx*100)+'%" title="'+y+': '+fmtCr(tot)+'">';
   ...
   h+='</div></div><div class="yr">'+y+'</div></div>';});
 h+='</div>';
 if(domsHere.length)h+='<div class="chips" style="margin-top:8px">'+domsHere.map(dm=>'<span class="chip" style="border-left:3px solid '+domainPalette[dm]+'">'+shortDom(dm)+'</span>').join('')+'</div>';
 h+='</div>';
 body.innerHTML=h;
